...
- **Clear OAuth cookies after use** — the CSRF state cookie must be cleared in the callback regardless of success or failure. The signed state carries its own expiry (`STATE_MAX_AGE_S`); `verifyState()` rejects a timed-out attempt even if the browser still presents the cookie.
- **Rate limit every route** — every function in `api/` calls `rateLimit()` with a per-IP budget before doing work; the budgets are documented in README section 12. Unauthenticated routes (`start`, `callback`, `ics`) and routes that spend the user's GitHub quota (`session`) must never be added without one.
...
- **Endpoint allowlist in proxy** — `api/github.ts` only forwards the exact endpoints the editor uses (`/user`, repo root, `contents/`, `git/ref(s)/heads/main`, `git/commits`, `git/blobs`, `git/trees`, `compare/`), and every write is validated to stay under `public/data/`, `public/images/` and `public/documents/`. A content editor must never be able to commit application code. Path checks alone cannot guarantee that for the Trees flow (a commit may reference any tree in the repository network, including one pushed to a fork), so `verifyRefUpdate()` is run before every `PATCH git/refs/heads/main`: the target commit must sit directly on the current `main` tip and its tree may differ from the parent's only in regular blobs under the content directories. Any new proxy call must be added explicitly, with a test in `api/__tests__/github.test.ts`.
- **Content file types are allowlisted** — Vercel serves `public/` by extension, so an uploaded `.html` or `.js` would be a same-origin page or script. `isContentFilePath()` in `api/github.ts` (`CONTENT_EXTENSIONS`) restricts every create/replace to `.json` under `public/data/`, image formats under `public/images/` and `.pdf`/`.doc`/`.docx` under `public/documents/`; deletions only need `isContentPath()`. A new upload type in the admin must be added there first (and mirrored in the client-side check, e.g. `ALLOWED_DOCUMENT_EXTENSIONS` in `KommunalpolitikEditor.tsx`).
- **Request bodies are key-allowlisted** — every write body the proxy forwards is checked with `hasOnlyKeys()`; GitHub fields the editor never sends (`author`, `committer`, `signature`, …) are rejected so commit identities cannot be forged. When the editor starts sending a new field, add it to the allowlist together with a test.
- **Push access is enforced server-side** — the repo is public, so `api/auth/callback.ts` and `api/auth/refresh.ts` require `permissions.push` on the repo (via `api/auth/access.ts`) in addition to the optional `ALLOWED_GITHUB_LOGINS` allowlist. Both resolve the login from GitHub with the freshly issued token — never from a cookie.
- **Identity cookie is signed** — `spd_user_login` is issued by `makeAuthCookies()` as an HMAC-signed, expiring value. Anything that needs the caller's login without a GitHub round-trip (e.g. `api/admin-presence.ts`) must go through `verifyLoginCookie()`; never read the raw cookie value, and never sign new cookie types without a distinct purpose in `api/auth/cookies.ts`.
