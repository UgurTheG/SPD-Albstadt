  ...
  return (
    <motion.div
      ...
      /* overflow-hidden intentionally removed from this element:
         having it on a GPU-composited transform target (translate/scale)
         causes sub-pixel clipping artifacts at rounded corners in dark mode.
         Clipping now lives only on the non-transforming image container. */
    ...
    >
      {/* rounded-2xl + overflow-hidden here clip the zooming image without
          conflicting with the parent's GPU transform.
          mask-image fades the bottom so the card's bg-gray-950 shows through
          — no separate overlay layer that can drift during animation. */}
      <div
        style={{
          WebkitMaskImage: 'linear-gradient(to top, transparent 0%, black 42%)',
          maskImage: 'linear-gradient(to top, transparent 0%, black 42%)',
        }}
        className="aspect-3/4 landscape-compact:aspect-4/3 overflow-hidden rounded-2xl"
      >
        {bildUrl ? (
          <img
            loading="lazy"
