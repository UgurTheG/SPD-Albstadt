...
export function diffTab(
  ...
  if (tab.type === 'kommunalpolitik') {
    ...
    for (const [id, { j, i }] of currById) {
      if (!origById.has(id)) {
        out.push({
          ...
          groupKey: 'jahre',
          itemIndex: i,
          itemLabel: (j.jahr as string) || `Jahr ${i + 1}`,
          after: j,
        })
      }
    }
    ...
    for (const [id, { j: currJ, i: ci }] of currById) {
      ...
      diffArray(
        dokumentFields,
        origJ.dokumente as Record<string, unknown>[] | undefined,
        currJ.dokumente as Record<string, unknown>[] | undefined,
        ['jahre', ci, 'dokumente'],
        `${lbl} – Dokumente`,
        `jahre.${id}.dokumente`,
        out,
      ...
      diffArray(
        personFields,
        origJ.kreisraete as Record<string, unknown>[] | undefined,
        currJ.kreisraete as Record<string, unknown>[] | undefined,
        ['jahre', ci, 'kreisraete'],
        `${lbl} – Kreisräte`,
        `jahre.${id}.kreisraete`,
        out,
    ...
    if (pendingImagePaths && pendingImagePaths.size > 0) {
      for (const { j: currJ, i: ci } of currById.values()) {
        ...
        ;(
          [
            {
              arr: currJ.gemeinderaete,
              section: 'gemeinderaete',
              sectionLabel: `${lbl} – Gemeinderäte`,
            },
            { arr: currJ.kreisraete, section: 'kreisraete', sectionLabel: `${lbl} – Kreisräte` },
          ] as { arr: unknown; section: string; sectionLabel: string }[]
        ).forEach(({ arr, section, sectionLabel }) => {
          ...
          ;(arr as Record<string, unknown>[]).forEach((p, pi) => {
            ...
            out.push({
              ...
              kind: 'modified',
              group: sectionLabel,
              groupKey: `jahre.${currJ.id}.${section}`,
              itemIndex: pi,
              itemLabel: (p.name as string) || `#${pi + 1}`,
              fieldKey: 'bildUrl',
