...
function ToggleField({
  ...
  return (
    ...
    >
      <div
        className={`w-12 h-7 rounded-full relative transition-all duration-300 ${value ? 'bg-linear-to-r from-spd-red to-spd-red-dark shadow-sm shadow-spd-red/25' : 'bg-gray-200 dark:bg-gray-700'}`}
      >
        <span
      ...
      <span
        className={`text-sm font-medium transition-colors ${value ? 'text-spd-red dark:text-red-400' : 'text-gray-400'}`}
      >
        {value ? 'Aktiv' : 'Inaktiv'}
      </span>
    </button>
