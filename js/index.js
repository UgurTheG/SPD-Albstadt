...
for (const contentKey of [".aktuelles", ".partei", ".fraktion", ".historie", ".kontakte"]) {
...
document.querySelector('.trigger_fraktion').onclick = function () {
    ...
    document.querySelectorAll(".selector ul li.active").forEach(f => f.classList.remove("active"))
...
parteimenu_parent.forEach(el => el.addEventListener('click', event => {
    event.currentTarget.parentElement.querySelectorAll(".active").forEach(f => f.classList.remove("active"))
...
document.querySelector("#toPartei").onclick = function() {
    ...
    document.querySelector("#partei").classList.add("active");
    document.querySelector("#parteiinfo").innerHTML = "Das sind die Mitglieder des SPD Albstadt Vorstandes, dieser wurde bei der Jahreshauptversammlung im Juli 2022gewählt. "
}
...
fraktionmenu_parent.forEach(ele => ele.addEventListener('click', event => {
    event.currentTarget.parentElement.querySelectorAll(".active").forEach(f => f.classList.remove("active"))
    event.target.classList.add("active")
  }));
