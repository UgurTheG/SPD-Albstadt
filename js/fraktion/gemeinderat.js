fetch('resources/data/fraktion/gemeinderat/gemeinderat.json')
  ...
  .then((data) => {
    data.forEach((profile) => {
      ...
      document.querySelector('.content_personen .flexbox .gemeinderate .contents').appendChild(cellDiv);
    });
    document.querySelector('.content_personen .flexbox .gemeinderate .contents').childNodes.forEach((gemeindeRatElement) => gemeindeRatElement.addEventListener('click', (gemeindeRatEntry) => {
      gemeindeRatEntry.currentTarget.classList.toggle('active');
    }));
  });
