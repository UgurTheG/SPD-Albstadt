...
- **Writes are confined to content _types_**: Vercel serves everything under `public/` by extension, so a `.html` or `.js` file in a content directory would be a same-origin page or script that the CSP (`script-src 'self'`) would run. Creating or replacing a file therefore also has to pass a per-directory allowlist — `public/data/` → `.json`, `public/images/` → `.webp`/`.jpg`/`.jpeg`/`.png`/`.gif`/`.avif`, `public/documents/` → `.pdf`/`.doc`/`.docx`. Deleting is allowed for any file in the content directories so legacy files can still be cleaned up.
- **Request bodies are allowlisted field by field**: `author`, `committer` and `signature` on commits (and every other field the editor never sends) are rejected, so nobody can put someone else's name under a commit on `main`.
